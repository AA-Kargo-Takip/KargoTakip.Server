...
internal sealed class KargoGetAllQueryHandler(
	...
	) : IRequestHandler<KargoGetAllQuery,
	IQueryable<KargoGetAllQueryResponse>>
{
	public Task<IQueryable<KargoGetAllQueryResponse>> Handle(KargoGetAllQuery request, CancellationToken cancellationToken)
	{
		var response = (from entity in kargoRepository.GetAll()
						join create_user in userManager.Users.AsQueryable() on entity.CreateUserId 
						equals create_user.Id
						join update_user in userManager.Users.AsQueryable() on entity.CreateUserId 
						...
						select new KargoGetAllQueryResponse
						{
							...
							CreateUserName = create_user.FirstName + " " + create_user.LastName +
							" (" + create_user.Email + ")",
							UpdateUserId = entity.UpdateUserId,
							UpdateUserName = entity.UpdateUserId == null ? null : 
